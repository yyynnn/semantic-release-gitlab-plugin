...
test.serial(
  'Verify token and repository access (project_access 30)',
  async (t) => {
    ...
    await t.notThrowsAsync(
      verify(
        {},
        {
          env,
          options: {
            repositoryUrl: `git+https://gitalb.com/${owner}/${repo}.git`,
          },
...
test.serial(
  'Verify token and repository access (project_access 40)',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
...
test.serial(
  'Verify token and repository access (group_access 30)',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
...
test.serial(
  'Verify token and repository access (group_access 40)',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
...
test.serial(
  'Verify token and repository access and custom URL with prefix',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
...
test.serial(
  'Verify token and repository access and custom URL without prefix',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
    ...
    t.deepEqual(t.context.log.args[0], [
      'Verify GitLab authentication (%s)',
      'https://othertesturl.com:9090/api/v4',
    ]);
  },
);

test.serial(
  'Verify token and repository access with subgroup git URL',
  async (t) => {
    ...
    await t.notThrowsAsync(
      verify(
        ...
        {
          env,
          options: { repositoryUrl: `git@customurl.com:${repoUri}.git` },
          logger: t.context.logger,
        },
      ),
    );
...
test.serial(
  'Verify token and repository access with subgroup http URL',
  async (t) => {
    const repoUri = 'orga/subgroup/test_user/test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
    const gitlabUrl = 'https://customurl.com:9090/context';
    ...
    await t.notThrowsAsync(
      verify(
        ...
        {
          env,
          options: { repositoryUrl: `http://customurl.com/${repoUri}.git` },
          logger: t.context.logger,
        },
      ),
    );
...
test.serial(
  'Verify token and repository access with empty gitlabApiPathPrefix',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = { GL_TOKEN: 'gitlab_token' };
...
test.serial(
  'Verify token and repository with environment variables',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {
      GL_URL: 'https://othertesturl.com:443',
...
test.serial(
  'Verify token and repository access with alternative environment varialbes',
  async (t) => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {
      GITLAB_URL: 'https://othertesturl.com:443',
      GITLAB_TOKEN: 'gitlab_token',
      GITLAB_PREFIX: 'prefix',
    };
    const gitlab = authenticate(env)
      .get(`/projects/${owner}%2F${repo}`)
      .reply(200, { permissions: { project_access: { access_level: 40 } } });
  ...
  const [error, ...errors] = await t.throwsAsync(
    verify(
      {},
      {
        env,
        options: {
          repositoryUrl: 'https://gitlab.com/semantic-release/gitlab.git',
        },
        logger: t.context.logger,
      },
    ),
...
test.serial(
  'Throw SemanticReleaseError for invalid repositoryUrl',
  async (t) => {
    const env = { GITLAB_TOKEN: 'gitlab_token' };
    ...
    const [error, ...errors] = await t.throwsAsync(
      verify(
        ...
        {
          env,
          options: { repositoryUrl: 'git+ssh://git@gitlab.com/context.git' },
          logger: t.context.logger,
        },
      ),
    );
...
test.serial(
  'Throw AggregateError if multiple verification fails',
  async (t) => {
    const env = {};
    const gitlabUrl = 'https://gitlab.com/context';
    const assets = 42;
