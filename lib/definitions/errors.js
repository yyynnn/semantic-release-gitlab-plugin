...
const [homepage] = 'https://github.com/semantic-release/gitlab#readme'.split(
  '#',
);
const linkify = (file) => `${homepage}/blob/master/${file}`;
const stringify = (obj) =>
  inspect(obj, { breakLength: Infinity, depth: 2, maxArrayLength: 5 });

function EINVALIDASSETS({ assets }) {
  return {
...
function EINVALIDGLTOKEN({ repoId }) {
  return {
...
function EGLNOPERMISSION({ repoId }) {
  return {
...
function ENOGLTOKEN({ repositoryUrl }) {
  return {
